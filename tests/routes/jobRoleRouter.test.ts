...
const getJobRoles = jobRoleService.getJobRoles as unknown as ReturnType<typeof vi.fn>;
...
describe("jobRoleRouter", () => {
	...
	it("GET /job-roles returns the job roles from the service", async () => {
		getJobRoles.mockResolvedValue([{ jobRoleId: 1, roleName: "Software Engineer" }]);

		const response = await request(createApp()).get("/job-roles");

		expect(response.status).toBe(200);
		expect(response.body).toEqual([{ jobRoleId: 1, roleName: "Software Engineer" }]);
	});
